...
public class HomeController {
    ...
    public void initData(User user, MainMenuController mainController) {
        ...
        if (this.currentUser != null) {
            GreetingText.setText("Hello, " + user.getFullName());

            /*
            if (user.getRecentProjects() != null) {
                recentProjectsList.setItems(FXCollections.observableArrayList(user.getRecentProjects()));
            }
    ...
    private void setupProjectListener(ListView<String> listView) {
        listView.getSelectionModel().selectedItemProperty().addListener((obs, oldVal, newVal) -> {
            if (newVal != null && mainController != null) {
                mainController.openProject(currentUser.getProjectByName(newVal));
